...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        idx = self._get_src_permutation_idx(indices)
        losses = {}
        losses['loss_bce'] = self.action_cls_loss(outputs_without_aux['pred_actions'], targets, indices, idx)
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                losses['loss_bce'+f'_{i}'] = self.action_cls_loss(aux_outputs['pred_actions'], targets, indices, idx)
